...
uint64_t ActivityStatsReaderImpl::downstreamActiveRequests() const {
  ...
  // Multiple protocols emit downstream_rq_active gauges:
  //   - http.<stat_prefix>.downstream_rq_active (HTTP Connection Manager)
  //   - redis.<stat_prefix>.downstream_rq_active (Redis Proxy)
  //   - generic_proxy.<stat_prefix>.downstream_rq_active (Generic Proxy)
  // We match any gauge ending with ".downstream_rq_active" to capture all protocols.
